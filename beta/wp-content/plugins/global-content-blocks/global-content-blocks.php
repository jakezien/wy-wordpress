<?php
	...
	*/
    ...
    if($current_version==0) {
      ...
      //just add all the updates here, they will be run only once anyways
      gcb_add_db_column("type", "VARCHAR(100) NOT NULL DEFAULT 'other'"); 
      gcb_add_db_column("custom_id", "VARCHAR(50) NOT NULL DEFAULT '' AFTER `id`");
	...
	function gcb_submenu() {
		...
		if(isset($_POST["gcb_save"])) {
			$name = $_POST["gcb_name"];
      $custom_id = mysql_real_escape_string(htmlspecialchars(sanitize_title_with_dashes($_POST['gcb_custom_id'])));
			...
			$value = mysql_real_escape_string(htmlspecialchars($_POST['gcbvalue']));
          ...
          custom_id='".$custom_id."' 
				...
				{
					$wpdb->query("insert into ".$wpdb->prefix . "gcb (name,description,value,type,custom_id) VALUES ('".$name."','".$description."','".$value."','".$type."','".$custom_id."')");
					$msg = "Entry inserted!";
				}
			}
			else
	...
	function gcb_import() {
      ...
      if(isset($row[4])) {
        $entries[count($entries)-1]["custom_id"] = mysql_real_escape_string(sanitize_title_with_dashes(base64_decode($row[4])));
      } else {
		...
		foreach($entries as $e) {
			$wpdb->query("insert into ".$wpdb->prefix . "gcb (name,description,value,type,custom_id) VALUES ('".$e["name"]."','".$e["description"]."','".$e["value"]."','".$e["type"]."','".$e["custom_id"]."')");
		}
	...
	*/
    ...
    if(is_numeric($id)) {
      ...
      $condition = "`custom_id` = '".$id."'";
    }
    
		//does this one exist ?
		$ex = $wpdb->get_var("SELECT COUNT(*) from `".$wpdb->prefix."gcb` WHERE ".$condition.";");
		if($ex==1) {
			$record = $wpdb->get_row("SELECT value,type FROM `".$wpdb->prefix."gcb` WHERE ".$condition.";");
			
			
			$content = htmlspecialchars_decode(stripslashes($record->value));
